...
export default function ProjectDetail() {
 ...
 useEffect(() => { document.title = (project ? project.title : 'Project not found') + ' · Project Zeal'; window.scrollTo(0, 0); heading.current?.focus(); }, [project]);
 if (!project) return <main className="portfolio-shell project-detail"><div className="portfolio-content"><h1 ref={heading} tabIndex={-1}>Project not found</h1><Link className="project-cta" to="/portfolio">Back to selected work</Link></div></main>;
 const next = projects[(projects.indexOf(project) + 1) % projects.length];
 return <main className="portfolio-shell project-detail" id="main-content"><div className="portfolio-content"><Link className="back-link" to="/portfolio">← Selected work</Link>
 ...
 <Link className="next-project" to={'/portfolio/' + next.slug}><span className="eyebrow">NEXT PROJECT</span><strong>{next.title} <span aria-hidden="true">→</span></strong></Link></div></main>;
}
